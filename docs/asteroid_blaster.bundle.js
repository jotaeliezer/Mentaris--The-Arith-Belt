...
(() => {
  ...
  function setShipIdle(state2, on) {
    if (!state2.sound) {
      if (shipIdleLoop)
        shipIdleLoop.pause();
      return;
  ...
  function setShipAdvance(state2, on) {
    if (!state2.sound) {
      if (shipAdvanceLoop)
    ...
    try {
      ...
      if (on) {
        if (shipAdvanceLoop.paused)
          shipAdvanceLoop.play();
      } else {
        shipAdvanceLoop.pause();
      }
    } catch (e) {
  ...
  function createTourGuide(options) {
    var opts = options || {};
    var gameShell2 = opts.gameShell || document.body;
    ...
    var steps = [
      ...
      { id: "move_up", title: "Step 1: Move Up", body: "Press W or Up Arrow to move up.", event: "move_up", hideDuringAction: true, actionPauseMs: 700, praiseTitle: "Altitude secured, cadet.", praiseBody: "Clean ascent. The fleet approves." },
      { id: "move_down", title: "Step 2: Move Down", body: "Press S or Down Arrow to move down.", event: "move_down", hideDuringAction: true, actionPauseMs: 700, praiseTitle: "Descent confirmed.", praiseBody: "Textbook drop. Gravity loses again." },
      { id: "move_left", title: "Step 3: Move Left", body: "Press A or Left Arrow to move left.", event: "move_left", hideDuringAction: true, actionPauseMs: 700, praiseTitle: "Portside shift executed.", praiseBody: "Nice slide - keeps you off enemy scopes." },
      { id: "move_right", title: "Step 4: Move Right", body: "Press D or Right Arrow to move right.", event: "move_right", hideDuringAction: true, actionPauseMs: 700, praiseTitle: "Starboard move clean.", praiseBody: "Witty sidestep. Keep them guessing." },
      { id: "fire_once", title: "Step 5: Fire Once", body: "Tap Z or click to fire a single shot.", event: "fire", count: 1 },
      { id: "fire_again", title: "Step 6: Fire Again", body: "Tap again to fire another shot.", event: "fire", count: 1 },
    ...
    for (var s = 0; s < steps.length; s++) {
      if (steps[s].showProgress === false)
        continue;
      progressTotal += 1;
    }
    var progressIndex = 0;
    for (var s2 = 0; s2 < steps.length; s2++) {
      if (steps[s2].showProgress === false)
        continue;
      progressIndex += 1;
    ...
    function mount() {
      ...
      gameShell2.appendChild(overlay);
    ...
    function handleStepCompletion(step) {
      if (!step || completedSteps[step.id])
        return;
  ...
  function drawAliens(ctx2) {
    ...
    for (var i = 0; i < aliens.length; i++) {
      ...
      if (sprite && sprite.img && sprite.img.complete && sprite.img.naturalWidth) {
        ...
        ctx2.beginPath();
        ctx2.arc(a.x + shakeX, a.y + shakeY, size * 0.3, 0, Math.PI * 2);
        ctx2.fill();
      }
      ctx2.restore();
  ...
  function loadWideGameplay() {
    var stored = null;
    try {
      stored = localStorage.getItem(wideGameplayKey);
    } catch (e) {
    }
    var isWide = stored === "1" || stored === "true";
    setWideGameplay(isWide);
    if (toggleWideGameplay)
      toggleWideGameplay.checked = isWide;
  }
  function setMousepadAutoStart(isOn) {
    mousepadAutoStart = !!isOn;
    try {
      localStorage.setItem(mousepadAutoKey, mousepadAutoStart ? "1" : "0");
    } catch (e) {
    }
  }
  function loadMousepadAutoStart() {
    var stored = null;
    try {
      stored = localStorage.getItem(mousepadAutoKey);
    } catch (e) {
    }
    mousepadAutoStart = stored === "1" || stored === "true";
    if (toggleMousepadAuto)
      toggleMousepadAuto.checked = mousepadAutoStart;
  }
  ...
  var powerupCatalogDefense = [
    ...
    { id: "armor", label: "Armor", icon: powerupIcons.armor.src, desc: "Temporary armor for 12 seconds." }
  ];
  ...
  function renderSettingsCatalogs() {
    ...
    renderCatalogList(sfxList, sfxCatalog, function(item) {
      if (!activeSfxCategory || activeSfxCategory === "all")
        return true;
  ...
  function getActiveProfileId() {
    ...
    var stored = null;
    try {
      stored = localStorage.getItem(campaignProfileKey);
    } catch (e) {
      stored = null;
    }
  ...
  function canMigrateLegacyCampaign() {
    ...
    if (pid !== "profile1")
      return false;
    try {
      return localStorage.getItem("mentaris.campaign.migrated." + pid) !== "1";
    } catch (e) {
      return true;
    }
  }
  function markLegacyCampaignMigrated() {
    var pid = getActiveProfileId();
    try {
      localStorage.setItem("mentaris.campaign.migrated." + pid, "1");
    } catch (e) {
    }
  }
  ...
  function getStoredCampaignId() {
    try {
      return localStorage.getItem(getCampaignActiveKey()) || "";
    } catch (e) {
      return "";
    }
  }
  function setStoredCampaignId(id) {
    try {
      localStorage.setItem(getCampaignActiveKey(), id);
    } catch (e) {
    }
  }
  ...
  function openMissileInput() {
    if (missileInputActive)
      return false;
    if (!missileOverlay || !missileInput)
      return false;
    if (!state.running || state.paused || state.over)
      return false;
    if ((player.blasterMode || "single") !== "missile")
    ...
    state.slowMoRemaining = Math.max(state.slowMoRemaining || 0, 5);
  ...
  function closeMissileInput(message) {
    ...
    if (missileOverlay)
      missileOverlay.classList.remove("show");
    ...
    if (p.group === "offense") {
      ...
      if (p.type === "repair") {
        ...
        for (var ai = 0; ai < asteroids.length; ai++) {
          var a = asteroids[ai];
          if (!a.isCorrect || a.waveId !== state.waveId) {
            targets.push({ id: a.id, y: a.y });
          }
        }
        targets.sort(function(a1, a2) {
          return a2.y - a1.y;
        });
        state.empCascade = targets;
        state.empCascadeTimer = 0;
        state.empCascadeInterval = 1;
      } else {
  ...
  function bumpContinueDifficulty() {
    ...
    if (inputs.speed && inputs.speed.options && inputs.speed.options.length) {
      ...
      if (speedIndex >= 0 && speedIndex < inputs.speed.options.length - 1) {
        addOption("SPEED", function() {
          inputs.speed.selectedIndex = speedIndex + 1;
        });
      }
    }
    if (inputs.decoys && inputs.decoys.options && inputs.decoys.options.length) {
      ...
      if (decoyIndex >= 0 && decoyIndex < inputs.decoys.options.length - 1) {
        addOption("DECOYS", function() {
          inputs.decoys.selectedIndex = decoyIndex + 1;
        });
      }
    }
    if (inputs.lives && inputs.lives.options && inputs.lives.options.length) {
      ...
      if (livesIndex > 0) {
        addOption("LIVES", function() {
          inputs.lives.selectedIndex = livesIndex - 1;
        });
      }
    }
    if (inputs.strikes && inputs.strikes.options && inputs.strikes.options.length) {
      ...
      if (strikesIndex > 0) {
        addOption("STRIKES", function() {
          inputs.strikes.selectedIndex = strikesIndex - 1;
        });
      }
    }
    ...
    if (diffIndex >= 0 && diffIndex < diffOrder.length - 1) {
      addOption("DIFFICULTY", function() {
        state.difficulty = diffOrder[diffIndex + 1];
      });
    }
    if (!options.length)
      return "";
  ...
  function openSettings() {
    ...
    renderSettingsCatalogs();
    if (sfxCatalogPanel)
      sfxCatalogPanel.classList.add("open");
  }
  function closeSettings() {
    ...
    if (overlayControls)
      overlayControls.classList.remove("show");
    if (sfxCatalogPanel)
      sfxCatalogPanel.classList.remove("open");
    stopAllPreviewAudio();
  ...
  function update(dt) {
    ...
    if (missileInputActive) {
      ...
      var remainingSec = Math.max(0, remainingMs / 1e3);
      if (missileTimerEl) {
    ...
    if (tourGuide && tutorialActive) {
      ...
      if (moveDist > 1.5) {
        if (Math.abs(dxMove) >= Math.abs(dyMove)) {
          if (dxMove > 1.5)
            tourGuide.notify("move_right");
          if (dxMove < -1.5)
            tourGuide.notify("move_left");
        } else {
          if (dyMove > 1.5)
            tourGuide.notify("move_down");
          if (dyMove < -1.5)
            tourGuide.notify("move_up");
        }
    ...
    if (state.empCascade && state.empCascade.length) {
      ...
      if (state.empCascadeTimer <= 0) {
        ...
        if (removed) {
          var nextInterval = state.empCascadeInterval > 0 ? state.empCascadeInterval : 1;
  ...
  function draw() {
    ...
    if (!state.over) {
      ...
      if (!(tutorialActive && tutorialHideQuestion)) {
  ...
  function drawTutorialPortal() {
    if (!tutorialActive || !tutorialPortalActive)
      return;
  ...
  function drawAsteroid(a) {
    ...
    if (sprite && asteroidSpriteReady[a.spriteIndex]) {
      ctx.save();
      ctx.globalAlpha = (a.ghost ? 0.2 : 0.95) * fadeAlpha;
  ...
  if (missileInput) {
    missileInput.addEventListener("input", function() {
      ...
      if (cleaned !== missileInput.value)
        missileInput.value = cleaned;
  ...
  function applyQueryParams() {
    ...
    if (campaignParam === "1" || campaignParam === "true") {
      ...
      if (profileParam) {
        ...
        try {
          localStorage.setItem(campaignProfileKey, profileParam);
        } catch (e) {
        }
      }
